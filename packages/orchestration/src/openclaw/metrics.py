...
# ---------------------------------------------------------------------------
# collect_metrics: original public API — creates JarvisState, reads from disk
# DANGER: Must NOT be called from inside _write_state_locked (deadlock risk).
# ---------------------------------------------------------------------------
...
def collect_metrics(project_id: str) -> dict:
    ...
    WARNING: This function instantiates JarvisState and calls read_state()
    internally. It MUST NOT be called from inside _write_state_locked.
    Use collect_metrics_from_state(state_dict) instead in that context.
    """
    try:
        # Lazy import to avoid circular dependency:
        # state_engine imports metrics, so metrics must not import state_engine at module level.
        from .state_engine import JarvisState  # noqa: PLC0415
    ...
    return {
        ...
        "pool": {
            # In a real environment, pool active count might come from pool registry.
            # Simplified static max reading for now.
            "active_containers": len([t for t in tasks.values() if t.get("status") == "in_progress"]),
