...
description: Use when running Codex CLI, Claude Code, Gemini CLI, OpenCode, or Pi Coding Agent via background process for programmatic control. Automates coding tasks with PTY mode for interactive terminal applications.
---
...
**Fallback when Codex unavailable.** See `references/claude-code.md` for detailed docs.
...
**Alternative fallback with different model family.** See `references/gemini-cli.md` for detailed docs.
...
1. **PTY is essential** — Without `pty:true`, output breaks or agent hangs.
2. **Git repo required** — Codex won't run outside a git directory. Use `mktemp -d && git init` for scratch work.
3. **exec is your friend** — `codex exec "prompt"` runs and exits cleanly.
4. **submit vs write** — Use `submit` to send input + Enter, `write` for raw data.
5. **Always use GPT-5.2-codex high** for complex work: `--model gpt-5.2-codex-high`
6. **Clear prompts** — Include issue number, description, expected outcome, test instructions
7. **Atomic commits** — Tell Codex to commit after each logical change
8. **Push early** — Push to remote branch so progress isn't lost if session dies
9. **Checkpoint logs** — Capture tmux output periodically to files
10. **Self-heal aggressively** — If stuck >10 mins, intervene automatically
